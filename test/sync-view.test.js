...
test('shows PENDING, OFFLINE and SYNC ERROR for the other three states', () => {
  assert.equal(computeSyncView({ state: 'pending' }).label, '◈ PENDING');
  assert.equal(computeSyncView({ state: 'offline' }).label, '◈ OFFLINE');
  assert.equal(computeSyncView({ state: 'error' }).label, '◈ SYNC ERROR');

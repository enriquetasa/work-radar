...
// authService.onChange() below. `syncEngineRunning` mirrors that so
// data:save (below) knows whether to route through the engine's
// save-race-safe recordLocalSave or fall back to a plain write.
let syncEngine = null;
...
ipcMain.handle('data:save', async (_e, data) => {
  ...
  try {
    if (syncEngineRunning) {
      ...
      // docs/supabase-sync-plan.md's Phase 4 notes ("the save race") for
...
async function createWindow() {
  ...
  // docs/supabase-sync-plan.md's Phase 4 notes) — a no-op while sync
